...
let bassLightColor = new THREE.Color(0x312ee8)
...
let osc1 = new Tone.Oscillator(0, 'sawtooth').connect(gain)
let osc2 = new Tone.Oscillator(0, 'sawtooth').connect(gain)
let osc3 = new Tone.Oscillator(0, 'sawtooth').connect(gain)
let osc4 = new Tone.Oscillator(0, 'sawtooth').connect(gain)
...
let bassSequence = Array(16).fill(0)
...
const loopCounter = new Counter(0, newLoopCounterMax(), () => {
  ...
  drunk.tick()
  const note = notes[0] + drunk.val
  osc3.frequency.value = Tone.Frequency.mtof(note - 12)
  osc4.frequency.value = Tone.Frequency.mtof(note - 17)
  lightColor.copy(violet)
  lightColor.lerp(
    gold,
    (drunk.val - drunk.minVal) / (drunk.maxVal - drunk.minVal)
...
function generateSequence() {
  for (let i = 0; i < sequence.length; i++) {
    ...
    bassSequence[i] = Math.random() > 0.5
  }
  ...
  bassPattern.length = 0
  for (let i = 0; i < 3; i++) {
    bassPattern.push(patternRandomizer.next())
  }
}
...
let stepLoop = new Tone.Loop(function (time) {
  ...
  const bassMute = bassSequence[stepVal]
  osc3.mute = bassMute
  osc4.mute = bassMute
  Tone.Draw.schedule(function () {
    ...
    if (!bassMute) {
      bassPattern.forEach((index) => {
        materials[index].color = bassLightColor
